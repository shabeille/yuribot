...
class RepeatView(discord.ui.View):
    ...
    async def repeat_callback(self, _, interaction):
        await self.cog.send_yuri(
            interaction.followup.send, self.tags_list, interaction.response.defer
        )
...
class YuriBotCog(discord.Cog):
    ...
    async def autocomplete_yuri(ctx: discord.AutocompleteContext):
        ...
        prefix = (
            ...
            )  # for all da preceding tagz
            + (
                "-" if len(tags[-1]) > 0 and tags[-1][0] == "-" else ""
            )  # for da blacklisted tag. meow :3 !!
        )
    ...
    async def yuri(self, ctx: discord.ApplicationContext, tags: str):
        ...
        tags_list: list = (
            [] if tags == "" else ([tag.strip() for tag in (tags[:index]).split(",")])
        )
    ...
    async def send_yuri(self, send, tags_list: list, defer):
        ...
        with io.BytesIO(image_bytes) as buffer:
            file = discord.File(buffer, f"yuri{os.path.splitext(image_url)[-1]}")
            ...
            embed = discord.Embed(
                title="Yuri!!!",
                description=f"Tags used: `{' '.join(tags_list)}`"
                if tags_list
                else None,
    ...
    async def tag_stats(self, ctx: discord.ApplicationContext, count: int):
        ...
        try:
            keys, vals = zip(
                *sorted(self.bot.stats.get_tags_used().items(), key=lambda x: x[1])
            )
